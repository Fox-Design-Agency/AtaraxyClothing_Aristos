...
const variants = require("../../plugins/printful/product_variants")
// GET product model
...
router.get("/pay/:orderid", (req, res) => {
    let total = req.session.cart.map((x, i, arr) => {
    ...
    Order.findById(req.params.orderid, function (err, order) {
        ...
        req.session.cart.map((x, i, arr) => {
            itemsOrder.push({ "quantity": x.qty, "variant_id": x.variant, "files": [{ "id": x.fileId }] })
        })
        if (err) {
            console.log(err)
        }
